...
- **Platform release pin**: there is no machine-readable pin — the Xcode project consumes SwiftDashSDK by path from the `../platform` sibling checkout, so the checkout's branch IS the pin. The release build must consume platform **`v4.2-dev`**, at or past dashpay/platform#4347 (asset-lock restore finality — without it, restored wallets render every historical funding tx as a pending transfer). Feature branches under development integration (currently `fix/asset-lock-restore-finality` until #4347 merges) are temporary; upstream all required changes before release. (The old `local/tx-decode-plus-v4.1-dev-dashwallet` / `v4.1-dev` pin is history.)
