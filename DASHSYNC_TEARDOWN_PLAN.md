...
| 7 | **Paste/NFC `dash:` URI with embedded amount is dead.** Non-ScanQR sources skip the amount screen; `confirmProtocolRequest`'s SDK gate needs `amount>0` (0 here) → falls into DashSync's zero-UTXO account → guaranteed "insufficient funds". | `DWPaymentProcessor.m:488` | Medium — real balance, false failure | **Mechanism fixed by C8 step 4** (`c1de1f831`) — `confirmPaymentIntent:` takes `self.amount` from `intent.amount` before the SDK gate, so the embedded amount routes through SwiftDashSDK, not the DashSync zero-UTXO branch. Confirm with a funded paste-with-amount smoke; the `change-amount`/`UINT64_MAX` cleanup still rides step 5. |
