...
**Progress 2026-07-07:** **A + B done** (with C1 categories 2/5 — commits `893458077` + `b09946166`, migration-doc row #25): `SwiftDashSDKHost.hasPersistedSDKWallet()` presence primitive, `WalletEnvironment.hasSDKWallet`/`hasWallet` (union), runtime gate flipped to SDK presence, 5 app-level existence reads repointed. Open: **C** (first-class reinstall recovery), **D** (`canWipeWithPhrase`/`isWalletEmpty`), **E** (dual-write deletion + scrubber, gated on C1 cat-3/6 + C10/C11).
...
~~`WalletEnvironment` build-out (C1 categories 2/5)~~ **done 2026-07-07** · ~~Transaction `.ds` deletion (C2)~~ **done 2026-07-06** · Payments zero-DS\* (C8 steps 2–5) · Auth replacement (C7, fixes Bugs #2/#3) · Utility tail — logger + **price pipeline** (C9) · CrowdNode signing app-side (C9/cov) · Uphold HTTP + keychain shims (cov) · Wallet presence + reinstall recovery (C6 A–D; **A+B done 2026-07-07**, C/D open) · ~~CrowdNode online-account forward (C3 PR2)~~ done · ~~APY calculator port (C5 PR2)~~ done.
