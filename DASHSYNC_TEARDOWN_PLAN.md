...
**Fourth hole found 2026-07-07 (C1 cat-4 sweep):** the About screen's trusted-peer debug feature (`DWAboutModel.m` `setTrustedPeerHost:`/`removeTrustedPeerHost` paths, ~:200-217) calls `[peerManager connect]` — another DashSync networking starter, alongside `switchToNetwork`. Left in place (debug surface, owner decision pending); record any fix with C7/C9 utility work.
