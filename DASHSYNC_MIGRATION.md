...
| **#3 Mnemonic generation** | 🌓 Shadow | Fresh-install and restored wallets now create a SwiftDashSDK wallet via `DWSwiftDashSDKWalletCreator` from `DWPreviewSeedPhraseModel.getOrCreateNewWallet`, called in parallel with the existing DashSync `[DSWallet standardWalletWithRandomSeedPhraseForChain:...]`. **No DashSync code paths were modified** — both libraries run independently. DashSync remains authoritative for every read consumer in the app; SwiftDashSDK is populated as a write store with no current readers. The mnemonic itself is still generated by DashSync's `DSBIP39Mnemonic` and read back via `seedPhraseIfAuthenticated`; PIN is read via `[DSAuthenticationManager getPin:]` (`+Private.h:51`). The new creator is pure SwiftDashSDK code (no DashSync coupling) — Obj-C glue lives in a private helper `createWalletInSwiftDashSDK` in the consumer file, easy to delete in a future PR when DashSync is removed. **Note:** the classic Stage 🌓 mismatch-logger pattern doesn't apply here because mnemonic generation is random (no deterministic output to compare); the "shadow" is on the write side only — both stores get populated in parallel, neither read is changed. **Next:** flip entropy generation to `Mnemonic.generate()` + `+[DSWallet standardWalletWithSeedPhrase:setCreationDate:forChain:storeSeedPhrase:isTransient:]` (`DSWallet.h:123`) so SwiftDashSDK becomes the entropy source. |
