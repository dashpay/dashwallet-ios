...
| **#4 Mnemonic validation** | 🌗 Flipped | Adapter `DWSwiftDashSDKMnemonicValidator` calls both `[DSBIP39Mnemonic phraseIsValid:]` and `Mnemonic.validate(_:)` on every invocation, **returns SwiftDashSDK's result** (now authoritative), and continues to log any disagreements via `swift-sdk-migration.mnemonic-validator` as a safety net. Single call site rewired in `DWRecoverModel.m:79` — the model wrapper is the only place that touches `phraseIsValid:`, and `DWRecoverContentView.m:219` continues to call into it unchanged. Stage 0 → Stage 1 was rolled into one commit after a manual smoke test in the Restore Wallet flow produced zero `phraseIsValid mismatch` warnings on a known-good 12-word phrase. Doc previously listed `DWPhraseRepairViewController.m` as a call site — that was incorrect (that file uses `findPotentialWordsOfMnemonic*` for spell-check repair, not validation), corrected here. Sister `DSBIP39Mnemonic` wrappers in `DWRecoverModel.m` (`cleanupPhrase:`, `normalizePhrase:`, `wordIsLocal:`, `wordIsValid:`) have **no** SwiftDashSDK equivalents in current `Mnemonic.swift` and remain on DashSync. **Next:** Stage 2 (drop the DashSync parallel call entirely) after the flipped adapter has had a chance to bake. |

Everything else in the table below is `—` (not started). The SwiftDashSDK package itself is linked and building (commits `8309e1ef4` for the SPM integration, `19b475684` for the first migration adapter, `3c87467e5` for the first Stage-2 cleanup).
...
| 4 | **Mnemonic validation** (Restore Wallet) | `Sources/UI/Setup/RecoverWallet/DWRecoverModel.m` (the wrapper, called from `DWRecoverContentView.m`) | `[DSBIP39Mnemonic phraseIsValid:]` | `Mnemonic.validate(_:)` via `DWSwiftDashSDKMnemonicValidator` | 🟢 | 🌗 Flipped | High | S | None |
