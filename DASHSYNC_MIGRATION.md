...
|---|---|---|
| **#2 Address validation** | 🌓 Shadow | Adapter `DWSwiftDashSDKAddressValidator` shipped in commit `19b475684`. Both libraries are now called on every Send-screen address parse; DashSync remains authoritative. Mismatches log to `os.log` subsystem `org.dashfoundation.dash`, category `swift-sdk-migration.address-validation`. **Next:** monitor logs for ~1 release; if clean, follow-up PR flips the adapter to return `sdkResult` instead of `dashSyncResult` (one-line change). |
...
| 2 | **Address validation** (paste/scan in Send screen) | `Sources/UI/Payments/PaymentModels/DWPaymentInputBuilder.m`, `DWPaymentProcessor.m` | `[NSString isValidDashAddressOnChain:]` | `Address.validate(_:network:)` via `DWSwiftDashSDKAddressValidator` | 🟢 | 🌓 Shadow | High | S | None |
