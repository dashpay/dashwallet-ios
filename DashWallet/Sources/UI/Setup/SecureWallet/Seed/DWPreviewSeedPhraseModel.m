...
/// The phrase generated by this model for a NEW wallet. Persistence is async
/// (`createSwiftDashSDKWalletWithMnemonic:`), so repeat `getOrCreateNewWallet`
/// calls (backup-info → preview screen) must hand back the SAME phrase rather
/// than racing `hasWallet` and generating a second one.
@property (nullable, nonatomic, copy) NSString *generatedSeedPhrase;
...
- (DWSeedPhraseModel *)getOrCreateNewWallet {
    ...
    if (self.generatedSeedPhrase.length > 0) {
...
- (void)clearAllWallets {
    // The next `getOrCreateNewWallet` must generate a fresh phrase (same
    // word count), not replay the one that was just screenshotted.
    self.generatedSeedPhrase = nil;
