...
extension PointOfUseListFiltersViewController: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if let identifier = identifier, identifier == .locationService {
            UIApplication.shared.open(URL(string:UIApplication.openSettingsURLString)!)
            return
        }
        
        if let identifier = identifier, identifier == .resetFilters {
            model.resetFilters()
            updateApplyButton()
            updateResetButton()
...
extension PointOfUseListFiltersViewController.DataSource {
    final func identifier(for section: Int) -> PointOfUseListFiltersViewController.Section? {
        ...
        if #available(iOS 15.0, *) {
            ...
            identifier = self.snapshot().sectionIdentifiers[section]
        }
