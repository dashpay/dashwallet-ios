    ...
    private func configureHeaderView() {
        ...
        if case let .merchant(m) = merchant.category {
            if let currentLocation = DWLocationManager.shared.currentLocation, DWLocationManager.shared.isAuthorized {
                ...
                subLabel.text = "\(ExploreDash.distanceFormatter.string(from: Measurement(value: floor(distance), unit: UnitLength.meters))) · Physical Merchant" + (m.type == .onlineAndPhysical ? ", Online" : "")
            }else{
