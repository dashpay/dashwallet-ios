...
extension PointOfUseDetailsView {
    ...
    @objc func configureActionBlock() {
        ...
        if merchant.phone != nil {
            let button = actionButton(title: NSLocalizedString("Call", comment: "Call"), icon: "phone.circle.fill", action: #selector(callAction))
            buttonsStackView.addArrangedSubview(button)
        }
        
        if merchant.showMap {
            let button = actionButton(title: NSLocalizedString("Direction", comment: "Direction"), icon: "arrow.triangle.turn.up.right.circle.fill", action: #selector(directionAction))
            buttonsStackView.addArrangedSubview(button)
        }
        
        if merchant.website != nil {
            let button = actionButton(title: NSLocalizedString("Website", comment: "Website"), icon: "safari.fill", action: #selector(websiteAction))
            buttonsStackView.addArrangedSubview(button)
        }
