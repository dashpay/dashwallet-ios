...
@objc class AddressUserInfoDAOImpl: NSObject, AddressUserInfoDAO {
    ...
    @objc func create(dto: AddressUserInfo) {
        do {
            let userInfo = AddressUserInfo.table.insert(or: .replace, AddressUserInfo.addressColumn <- dto.address, AddressUserInfo.txCategoryColumn <- dto.taxCategory.rawValue)
            try db.run(userInfo)
    ...
    @objc static let shared: AddressUserInfoDAOImpl = AddressUserInfoDAOImpl()
}
