...
| Wave-1 function adapters consume the migrated per-network runtime descriptors (e.g. #1 receive address, #11/#12 runtime start) | 🌗 In progress | Runtime-owned SPV startup and mainnet/testnet switching now consume `wallet.descriptor.mainnet` / `wallet.descriptor.testnet`; more Wave-1 adapters still pending. |
...
The migrator uses the lowest-level public SwiftDashSDK API surface indirectly through `SwiftDashSDKRuntimeDescriptorFactory`: a standalone `WalletManager(network:)` (which calls `wallet_manager_create` directly with `ownsHandle = true` — no `SPVClient` or `CoreWalletManager`) builds the wallet bytes via `addWalletAndSerialize`, then best-effort `ensurePlatformPaymentAccount` runs, and the resulting descriptor is written under the current network's app-owned Keychain account. Around that, the migrator still does `Mnemonic.toSeed` + `WalletStorage().storeSeed(seed, pin:)`, round-trip `retrieveSeed(pin:)`, and `storeMnemonic`/`retrieveMnemonic` verification before the network-scoped runtime descriptor write/read-back. The `WalletManager` local still goes out of scope naturally once the factory returns and ARC frees the FFI handle.
